...
#   candidate.nodes$root <- ghost.nodes$branch
  candidate.nodes$branch <- 1:dim(candidate.nodes)[1]
  candidate.nodes$root <- 1:dim(candidate.nodes)[1]
...
#   #remove candidate nodes from non.candidate nodes dataframe 
#   non.candidate.nodes <- subset(non.candidate.nodes,
#                                 !(group %in% new.candidate.nodes$group &
#                                      order %in% new.candidate.nodes$order)
#                                 )
  ...
  branch_identify <- function(candidate_df, non_candidate_df)
  {
    ...
    while (dim(candidate_df)[1] > 0) 
    {
      ...
      candidate_df <- subset(candidate_df, !(MV.line.per.kwh == min(MV.line.per.kwh)))
