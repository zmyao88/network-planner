...
# ## 5.0 Import & fortify existing grid .shp file -> existing.grid
# existing <- readOGR(folder, "networks-existing")
# existing.fortified <- fortify(existing) ## very very slow process! 
# #assume any node on this network is "fake.node" since it doesn't corrspond to new connected node
# existing.fortified$fake.nodes <- FALSE 
# merged$fake.nodes <- TRUE 
# #while we're at it, bring in other shape files too...maybe
# #nodes <- readOGR(folder, "nodes")
# 
# ## 6.0 Merge existing.grid vertex nodes with merged vertex nodes ensuring ALL merged nodes are included -> non.candidate.nodes 
# non.candidate.nodes <- merge(merged, existing.fortified, all.x = T, all.y=F)
# 
# ## 7.0 Find and label all nodes without associated settlement points as "fake.nodes" in non.candidate.nodes
# non.candidate.nodes$fake.node <- (TRUE)  
# non.candidate.nodes$fake.node <- is.na(non.candidate.nodes$Name)  
...
# ghost.nodes <- ddply(ghost.nodes, .(group))
...
# candidate.nodes <- ddply(candidate.nodes, .(group))
##candidate.nodes <- cbind(candidate.nodes, branch)
...
while (length(new.candidate.nodes)>0) {
    candidate <- subset(new.candidate.nodes, MV.line == min(MV.line))
    ...
    ranked.settlements <- rbind.fill(ranked.settlements, candidate)
    ...
    new.segments <- non.candidate.nodes$id[which((non.candidate.nodes$Name %in% ranked.settlements$Name) & !(non.candidate.nodes$id %in% candidate$id))]
    if (length(new.segments)>0) 
    {
        segments <- rbind(segments, new.segments)
        ...
        new.dist <-  distVincentySphere(candidate[,2:3],
                                        new.candidate.nodes2[,1:2],
                                        r=6371010) #radius of Earth Network Planner uses
        ...
        new.candidate.nodes <- rbind.fill(new.candidate.nodes, new.candidate.nodes2)
